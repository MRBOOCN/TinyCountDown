...
function createTimeOptions() {
	return [
		...
		{ type: 'number', label: '秒', id: 'seconds', default: 0, min: 0, max: 59 },
	]
}
...
class TinyCountdownInstance extends InstanceBase {
		...
		return [
			{
				...
				label: '信息',
				value:
					'<strong>TinyCountdown 模块</strong><br/>通过 Companion 控制您的 TinyCountdown 应用。<br/>请确保 TinyCountdown 正在运行且 Web 服务器已启用。',
			},
			{
				type: 'textinput',
				id: 'host',
		...
		this.setActionDefinitions({
			...
			set_ndi_fps: {
				...
				callback: async (action) => {
					...
					this.log('info', `操作：设置 NDI 帧率为 ${FPS_MAP[fps] || fps}`)
				},
			},
		})
	}
	...
	initFeedbacks() {
		this.setFeedbackDefinitions({
			start_stop_countdown: createBooleanFeedback(this, '开始/停止', { color: COLORS.green }, 'running'),
			stop_countdown: createBooleanFeedback(this, '停止状态', { color: COLORS.red }, 'running', true),
				...
				options: [
					{
						...
						id: 'mode',
						default: 'countdown',
						choices: MODE_STATUS_CHOICES,
					},
				],
				callback: (feedback) => {
					return this.connectionState.currentMode === feedback.options.mode
				},
			},
			time_remaining: {
				name: '剩余时间',
				...
				callback: (feedback) => {
					...
					// 仅在倒计时模式下判断剩余时间阈值
					if (this.connectionState.currentMode !== 'countdown') {
						return false
					}
					return this.connectionState.remainingTime <= threshold && this.connectionState.remainingTime > 0
				},
			},
			authenticated_status: createExpectedStateFeedback(
				this,
				'登录状态',
				{
					bgcolor: COLORS.green,
	...
	handleLogoutMessage(parsedData) {
		...
		if (reason === 'port_changed') {
			...
			this.log('warn', `TinyCountdown 端口已变更：${oldPort} -> ${newPort}，请在模块配置中更新端口并保存`)
			this.updateStatus(InstanceStatus.BadConfig, `端口已变更为 ${newPort}，请更新配置`)
		} else {
	...
	validateStatusData(data) {
		// 基本类型验证
		const log = this.log.bind(this)
		if (!validateRequiredBoolean(data, 'running', log)) return false
		if (!validateRequiredBoolean(data, 'paused', log)) return false
		if (!validateRequiredNumber(data, 'remainingTime', log)) return false
		if (!validateRequiredNumber(data, 'remainingTimeMs', log)) return false
		if (!validateRequiredNumber(data, 'totalTime', log)) return false
	...
	updateConnectionState(data) {
		...
		if (isRunning) {
			...
			this.stopInterpolation()
			// 非运行状态直接刷新显示
			const displayUpdates = this.updateRemainingTimeDisplay(
				Math.max(0, this.connectionState.remainingTimeMs),
				this.connectionState.currentMode,
			)
			Object.assign(updates, displayUpdates)
		}
		...
		// NDI 帧率
		if (data.ndiFps !== undefined && this.connectionState.ndiFps !== data.ndiFps) {
			this.connectionState.ndiFps = data.ndiFps
			updates.ndiFps = data.ndiFps.toString()
		}

		// 当前计时模式
		if (data.mode !== undefined && this.connectionState.currentMode !== data.mode) {
			this.connectionState.currentMode = data.mode
			updates.currentMode = capitalizeFirst(data.mode)
		}

		// 模式切换时刷新 time 变量格式，避免要等到下一秒才变化
		if (updates.currentMode !== undefined) {
			const displaySec = Math.max(0, Math.floor(this.connectionState.remainingTimeMs / 1000))
			const formattedTime = formatTimeVariable(displaySec, this.connectionState.currentMode)
			if (this.connectionState.time !== formattedTime) {
				this.connectionState.time = formattedTime
				updates.time = formattedTime
			}
		}

		// 时间模式始终需要本地插值来每秒刷新 time 变量（显示 TinyCountdown 服务端本地时间）
		if (this.connectionState.currentMode === 'time') {
			this.startInterpolation()
		}
	...
	updateRemainingTimeDisplay(remainingTimeMs, mode) {
		const displaySec = Math.max(0, Math.floor(remainingTimeMs / 1000))
		if (this.connectionState.remainingTime !== displaySec) {
			this.connectionState.remainingTime = displaySec
			const formattedTime = formatTimeVariable(displaySec, mode)
			this.connectionState.time = formattedTime
			return {
				remainingTime: displaySec.toString(),
				time: formattedTime,
			}
		}
		return {}
	...
	tickInterpolation() {
		if (this.connectionState.currentMode === 'time') {
			// 时间模式：以服务端最近一次同步的 remainingTimeMs 为基准，用本地时钟插值，
			// 显示 TinyCountdown 服务端本地时间，避免跨时区偏差且不额外请求网络
			const elapsedSec = Math.floor((Date.now() - this.connectionState.lastSyncTime) / 1000)
			const baseSec = Math.floor(this.connectionState.remainingTimeMs / 1000)
			const estimatedSec = (baseSec + elapsedSec) % 86400
			const formattedTime = formatTimeVariable(estimatedSec, 'time')
			if (this.connectionState.time !== formattedTime) {
				this.connectionState.time = formattedTime
				this.setVariableValues({ time: formattedTime })
				this.checkFeedbacks()
			}
			return
		}
		...
		let estimatedMs
		if (this.connectionState.currentMode === 'countup') {
			// 正计时：从最近一次同步的已计时间继续递增
			estimatedMs = this.connectionState.remainingTimeMs + elapsed
		} else {
			estimatedMs = Math.max(0, this.connectionState.remainingTimeMs - elapsed)
		}
		const displayUpdates = this.updateRemainingTimeDisplay(estimatedMs, this.connectionState.currentMode)
		if (Object.keys(displayUpdates).length > 0) {
			this.setVariableValues(displayUpdates)
			this.checkFeedbacks()
		}
	}
	...
	#shouldSkipReconnect() {
		return !this.isInitialized || !this.config.reconnect || this.closingIntentionally
	}
	...
	scheduleReconnectFromClose(closeCode = -1, closeReason = '') {
		...
		if (this.authToken && isTokenInvalidated(closeCode, closeReason)) {
			...
			this.#resetAuthAndReconnect('令牌失效，立即重新认证')
			return
		}
		...
		if (this.authToken && this.reconnectAttempt >= MAX_TOKEN_RECONNECT_ATTEMPTS) {
			...
			this.#resetAuthAndReconnect('旧令牌重连失败，重新认证')
			return
		}
